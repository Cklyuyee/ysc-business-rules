...
// Hash format: #rules | #rules/CUS | #usecases | #usecases/CRM
function parseHash(): { view: ViewTab; module: string } {
  const hash = window.location.hash.replace('#', '');
  const [v, mod] = hash.split('/');
  const view: ViewTab = v === 'usecases' ? 'usecases' : 'rules';
  return { view, module: mod || 'all' };
}
function pushHash(view: ViewTab, module: string) {
  const h = module === 'all' ? `#${view}` : `#${view}/${module}`;
...
export default function App() {
  ...
  const [activeModule, setActiveModule] = useState<string | 'all'>(initial.module);
  const [exporting, setExporting] = useState(false);

  // Sync state → hash whenever view/module changes
  useEffect(() => { pushHash(view, activeModule); }, [view, activeModule]);

  // Sync hash → state when user presses Back/Forward
  const onHashChange = useCallback(() => {
    const { view: v, module: m } = parseHash();
    setView(v); setActiveModule(m); setSearch('');
  }, []);
  useEffect(() => {
  ...
  const switchView = (v: ViewTab) => {
    setView(v); setActiveModule('all'); setSearch('');
  };
  const switchModule = (id: string) => {
    setActiveModule(id); setSearch('');
  };
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <button onClick={() => switchModule('all')}
            ...
            {currentModules.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => switchModule(isActive ? 'all' : m.id)}
