...
import {
  ...
  HelpCircle, BookOpen, Hash, Filter, Search, X,
} from 'lucide-react';
...
export default function App() {
  ...
  const handleExport = async () => {
    setExporting(true);
    try { await exportToDocx(); } catch (e) { console.error(e); } finally { setExporting(false); }
  };
    ...
    <div className="min-h-screen bg-neutral-50">
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-neutral-900">YSC Business Rules</h1>
            </div>
            <p className="text-xs text-neutral-500 mt-1 ml-11">
              {totalRules} rules | {USE_CASE_MODULES.length} modules | {totalQuestions} open questions
            </p>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative inline-flex items-center">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search rules, D-NNN..."
            ...
            {USE_CASE_MODULES.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setActiveModule(isActive ? 'all' : m.id)}
                  ...
                  <Icon className="w-3.5 h-3.5" />{m.id} ({m.rules.length})
                </button>
              );
            })}
          </div>
        ...
        <footer className="text-center py-8 text-xs text-neutral-400">
          YSC ERP — Business Rules | Source: team-knowledge-main (9 meetings)
        </footer>
      </main>
    </div>
  );
